...
function SignUpMetaMask(props: any) {
  ...
  const onSendCaptcha = async () => {
    ...
    try {
      let res = await axios.post(Urls.auth.sendEmailCaptcha(), {
        metamask: metamask,
        name: name,
        email: email,
      });
  ...
  const onCheck = async () => {
    ...
    try {
      let res = await axios.post(Urls.auth.signUpMetaMask(), {
        metamask: metamask,
        sign: result,
        name: name,
        email: email,
  ...
  const onNameInputBlur = async () => {
    ...
    try {
      let res = await axios.get(Urls.auth.checkUserExists(userName));
